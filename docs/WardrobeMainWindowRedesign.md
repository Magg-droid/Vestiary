## Plan: Wardrobe Main Window Visual Redesign

Use the first two screenshots as the current Wardrobe UI and the third screenshot as the visual target. The goal is a UI-only redesign that keeps the exact same features and interactions we already have. No pagination, no new workflow, no new feature set, and no service or data-model changes.
...
- Screenshot 1 and 2 = current Wardrobe UI
...
**Relevant files**
- `d:\Projects\Plugins\Wardrobe\Wardrobe\Windows\MainWindow\MainWindow.cs` — overall window composition and view switching
- `d:\Projects\Plugins\Wardrobe\Wardrobe\Windows\MainWindow\MainWindow.TabBar.cs` — collection tab rendering and drag/drop behavior
- `d:\Projects\Plugins\Wardrobe\Wardrobe\Windows\MainWindow\MainWindow.HeaderRow.cs` — search, counts, hidden toggle, and settings controls
- `d:\Projects\Plugins\Wardrobe\Wardrobe\Windows\MainWindow\MainWindow.Gallery.cs` — gallery container and empty state
- `d:\Projects\Plugins\Wardrobe\Wardrobe\Windows\MainWindow\MainWindow.DesignCard.cs` — card layout, thumbnail, actions, and button presentation
- `d:\Projects\Plugins\Wardrobe\Wardrobe\Windows\MainWindow\MainWindow.EmoteGallery.cs` — emote mode layout, which must remain functional and visually consistent
- `d:\Projects\Plugins\Wardrobe\Wardrobe\ClassicTheme.cs` — primary theme colors and visual tuning
- `d:\Projects\Plugins\Wardrobe\Wardrobe\OceanTheme.cs` — secondary theme consistency if the redesign needs shared palette changes
- `d:\Projects\Plugins\Wardrobe\Wardrobe\MidnightPurpleTheme.cs` — secondary theme consistency if the redesign needs shared palette changes
- `d:\Projects\Plugins\Wardrobe\Wardrobe\ForestTheme.cs` — secondary theme consistency if the redesign needs shared palette changes
- `d:\Projects\Plugins\Wardrobe\Wardrobe\ITheme.cs` — extend only if the redesign truly needs new shared color tokens

**Verification**
1. Build the Wardrobe solution and confirm there are no compile errors in the touched UI and theme files.
